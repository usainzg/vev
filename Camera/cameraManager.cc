...
CameraManager::~CameraManager() {
	for(map<string, Camera *>::iterator it = m_hash.begin(), end = m_hash.end();
		it != end; ++it)
		delete it->second;
}

OrthographicCamera *CameraManager::createOrthographic(const std::string &key) {
	map<string, Camera *>::iterator it = m_hash.find(key);
	if (it != m_hash.end()) {
...
Camera *CameraManager::find(const std::string & key) {
	map<string, Camera *>::const_iterator it = m_hash.find(key);
	if (it == m_hash.end()) return 0;
	return it->second;
}

void CameraManager::print() const {
	for(map<string, Camera *>::const_iterator it = m_hash.begin(), end = m_hash.end();
		it != end; ++it)
		it->second->print();
}
