...
void main() {
	...
	// obtenemos la información del brillo del mapa especular
	vec3 spec_map = texture2D(specmap, f_texCoord).rgb;

	// aplicamos la informacion obtenida del mapa 
	// especular en el calculo del color del fragmento
	vec4 color;
	color.rgb = scene_ambient + diffuse * texColor.rgb + specular * spec_map.rgb;
