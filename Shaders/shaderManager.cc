...
ShaderManager::~ShaderManager() {
	for(map<string, ShaderProgram *>::iterator it = m_hash.begin(), end = m_hash.end();
		it != end; ++it)
		delete it->second;
}
...
ShaderProgram *ShaderManager::create(const std::string &key,
	...
	map<string, ShaderProgram *>::iterator it = m_hash.find(key);
	if (it != m_hash.end()) {
...
ShaderProgram *ShaderManager::find(const std::string & key) const {
	map<string, ShaderProgram *>::const_iterator it = m_hash.find(key);
