...
void ShaderProgram::beforeDraw() {
	...
	if (mat != 0) {
		...
		// Add bump mapping capability
		if (has_capability("bump")) {
			tex = mat->getBumpMap();
			if (tex != 0) {
		...
		// Add specular mapping capability
		...
		// Add enviroment mapping capability
		...
		// Add shadow mapping capability
		if (has_capability("shadow")) {
			tex = rs->getShadowMap();
			if (tex != 0) {
				tex->bindGLUnit(Constants::gl_texunits::shadow);
				this->send_uniform("shadowmap", Constants::gl_texunits::shadow);
				this->send_uniform("worldToShadowCameraClip", rs->getLightC());
			}
		}
	}
}
