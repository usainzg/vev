...
float specular_factor(const vec3 n, const vec3 l, const vec3 v, float m) {
	...
	return (r_dot_v > 0) ? pow(r_dot_v, m) : 0.0;
}
...
void main() {
	// normal del vertice en coordenadas del mundo
	vec3 normalw = normalize(f_normalw);
	// calculamos el vector de pos_cam-pos_v (ambos sistema del mundo)
	vec3 I = normalize(campos - f_positionw);
	// vector para seleccionar el texel (reflejo del punto)
	vec3 R = 2 * dot(normalw, I) * normalw - I;
	// convertimos en levogiro
	R.z = -R.z;
	
	// obtenemos desde el texel el pixel seleccionado
	vec4 ref = textureCube(envmap, R);
